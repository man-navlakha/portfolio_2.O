...
import {
  ...
  FileText, ChevronLeft, ChevronDown, AlertCircle, Grid, Code2,
  Sparkles, Terminal, Square, ArrowUp, Menu, PenLine, Mic
} from 'lucide-react';
...
export default function ChatBot() {
  ...
  const parseFinalResponse = (fullText) => {
    ...
    const hireFormCopy =
      /quick form below/i.test(text) ||
      /open to freelance projects, collaborations, and full-time opportunities/i.test(text) ||
      /potential opportunity/i.test(text);
    if (hireFormCopy) {
      showHire = true;
    }
  ...
  }, [input, isStreaming, isWaiting, isVoiceMode, messages, startRevealTimer, stopRevealTimer]);
  ...
  return (
    ...
    ${isOpen
      ...
      <VoiceModeOverlay
        ...
        onCloseHireForm={() => setShowHireForm(false)}
        onHireSuccess={() => {
          setShowHireForm(false);
          const timeNow = new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true });
          setMessages(prev => [...prev, {
